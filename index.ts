...
class CardCounterApp extends AppServer {
  ...
  private async performScan(session: AppSession, state: any): Promise<void> {
    try {
      ...
      if (error && error.response && error.response.data) {
        console.error('[SCAN] Error (Base44 API response):', JSON.stringify(error.response.data));
      } else {
  ...
  private async detectCards(imageBase64: string): Promise<any[]> {
    ...
    if (!webhookUrl || !webhookSecret) {
      console.error('[BASE44] Webhook URL or secret not set in env!');
      return [];
    }

    try {
      console.log('[BASE44] Endpoint: ' + webhookUrl);
      console.log('[BASE44] Secret loaded: ' + (webhookSecret ? 'true' : 'false'));
      ...
      console.error('[BASE44] Fail data: ' + (error.response ? JSON.stringify(error.response.data) : 'No response'));
      return [];
    }
  }
